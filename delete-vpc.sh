...
# Veja a coluna "Descrição" de "Interfaces de rede" (que está em "Rede e segurança"). Encontre o item, dispositivo ou objeto relevante

set -e

if [ -z "$1" ]; then
    echo "Usage      : $0 <aws region> <vpc id> <profile"
    echo "For example: $0 us-east-1 vpc-xxxxxxxxxx admin"
    echo ""
    exit 1
fi
...
state=$(aws ec2 describe-vpcs \
    --vpc-ids ${VPC_ID} \
    --query 'Vpcs[].State' \
    --output text --region ${AWS_REGION} --profile ${profile}  --profile ${profile})
...
for instance in $(aws ec2 describe-instances \
    ...
    --output text --region ${AWS_REGION} --profile ${profile} )
do
